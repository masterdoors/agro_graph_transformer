...
def model_based_imputation_ds(ds_train, ds_test, fitter,Dataset,device,imp_test = False,random_ratio = None):
    ...
    for c,y in ds_train:
        for j,k in ds_train[c,y][1]:
            ds_train[(c,y)][1][j,k].append(ds_train[(c,y)][1][j,k][0] == 0)
            ds_train[(c,y)][1][j,k].append(ds_train[(c,y)][1][j,k][1] == 0)
            
            ds_train[(c,y)][1][j,k][0] = torch.from_numpy(imp.fit_transform(ds_train[(c,y)][1][j,k][0].numpy()))    
            ds_train[(c,y)][1][j,k][1] = torch.from_numpy(imp.fit_transform(ds_train[(c,y)][1][j,k][1]))

    for c,y in ds_test:
        for j,k in ds_test[c,y][1]:
            ds_test[(c,y)][1][j,k][0] = torch.from_numpy(imp.fit_transform(ds_test[(c,y)][1][j,k][0].numpy()))   
    ...
    test_len = dataset_test_.n_samples
    ...
    trainset, valset = torch.utils.data.random_split(dataset_train_, [train_len, val_len])
    ...
    for c,y in ds_train_:
        ...
        if fitter.model.wl_pos_enc:
            dataset_small._add_wl_positional_encodings()
                
        _,_,_,_,_,_,output = fitter.predict(dataset_small,dataset_small.collate)
                            ...
                            for mask_ratio in [0.1,0.3,0.5,0.7]:
                                for is_recurrent in [True, False]:
                                    ...
                                    for loss_type in loss_types:
                                        ...
                                        cluster_year_train_ = copy.deepcopy(cluster_year_train)
                                        cluster_year_test_ = copy.deepcopy(cluster_year_test)
                                        root_ckpt_dir = "checkpoints_encoder_decoder"
                                        ...
                                        net_params['lap_pos_enc'] = lap_pos_enc
                                        net_params['wl_pos_enc'] = wl_pos_enc
                                        net_params['max_cluster_size'] = max(len(dataset_train_.all_countries),len(dataset_train_.all_countries))
                                        net_params['is_recurrent'] = is_recurrent
                                        net_params['num_states'] = max(len(dataset_train_.all_countries),len(dataset_train_.all_countries))
                                        net_params['loss'] = loss_type    
