...
class TradeDGL(torch.utils.data.Dataset):
    ...
    def laplacian_positional_encoding(self,g, pos_enc_dim):
        ...
        if pos_enc.shape[1] < pos_enc_dim:
            ...
            #print("Ext on ", df)
        g.ndata['lap_pos_enc'] = pos_enc.to(self.device)
    ...
    def wl_positional_encoding(self,g):
        ...
        g.ndata['wl_pos_enc'] = torch.LongTensor(list(node_color_dict.values())).to(self.device)
        return g    
    
    def _add_laplacian_positional_encodings(self, pos_enc_dim):
        # Graph positional encoding v/ Laplacian eigenvectors
        self.graph_lists = [[self.laplacian_positional_encoding(g, pos_enc_dim) for g in gt] for gt in self.graph_lists]

    def _add_wl_positional_encodings(self):
        # WL positional encoding from Graph-Bert, Zhang et al 2020.
        self.graph_lists = [[self.wl_positional_encoding(g) for g in gt] for gt in self.graph_lists]
